...
void *thr_writer(void *_count) {
    ...
    while (!atomic_load(&stop_flag)) {
        ...
        *new = 8;
        ...
        urcu_memb_synchronize_rcu();  // Wait for all readers to finish
        if (old)
            *old = 0;
        free(old);
        nr_writes++;
...
void *thr_reader(void *_count) {
    ...
    urcu_memb_register_thread();  // Register the thread with RCU
    ...
    urcu_memb_unregister_thread();  // Unregister the thread from RCU
